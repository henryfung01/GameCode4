...
void CDXUTListBox::Render( float fElapsedTime )
{
    ...
    if( m_Items.GetSize() > 0 )
    {
		...
		// [mrmike] - I fixed the line below to remove the overlapping text problem in ListBoxes.
        // rc.bottom = rc.top + m_pDialog->GetManager()->GetFontNode( pElement->iFont )->nHeight;
        rc.bottom = rc.top + (m_pDialog->GetManager()->GetFontNode( pElement->iFont )->nHeight * 2);
