...
namespace LuaInterface
{
	...
	class CodeGeneration
	{
		 ...
		 */
		private Type GenerateEvent(Type[] eventHandlerTypes)
		{
			string typeName;
			lock(this) 
			{
			...
			Type[] paramTypes = new Type[eventHandlerTypes.Length];
			for (int i = 0; i < paramTypes.Length; ++i)
				paramTypes[i] = eventHandlerTypes[i];
...
//			generator.Emit(OpCodes.Ldarg_2);
		 ...
		 */
		public LuaEventHandler GetEvent(Type[] eventHandlerTypes, LuaFunction eventHandler)
		{
			Type eventConsumerType;
			if (eventHandlerCollection.ContainsKey(eventHandlerTypes)) 
			{
				eventConsumerType=eventHandlerCollection[eventHandlerTypes];
			} 
			else
			{
				eventConsumerType=GenerateEvent(eventHandlerTypes);
				eventHandlerCollection[eventHandlerTypes] = eventConsumerType;
			}
