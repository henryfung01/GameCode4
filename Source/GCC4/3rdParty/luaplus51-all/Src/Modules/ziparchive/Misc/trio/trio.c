/*************************************************************************
 *
 * $Id: trio.c,v 1.107 2007/11/11 13:21:51 breese Exp $
 *
...
static TRIO_CONST char rcsid[] = "@(#)$Id: trio.c,v 1.107 2007/11/11 13:21:51 breese Exp $";
	   ...
	   trio_pointer_t *argarray)
{
  ...
  while (format[offset])
      ...
      if (CHAR_IDENTIFIER == format[offset++])
	{
	  ...
	  flags = FLAGS_NEW;
	  dots = 0;
	  currentParam = TrioGetPosition(format, &offset);
	  ...
	  while (TrioIsQualifier(format[offset]))
		...
		case QUALIFIER_DOT:
		  if (dots == 0) /* Precision */
			  ...
			  )
			{
			  ...
			  precision = TrioGetPosition(format, &offset);
			...
			{
			  precision = trio_to_long(&format[offset],
						   &tmpformat,
						   BASE_DECIMAL);
			  offset = (int)(tmpformat - format);
			}
			  ...
			  )
			{
			  ...
			  base = TrioGetPosition(format, &offset);
			  if (base == NO_POSITION)
			    {
			      parameterPosition++;
			      if (positional)
			...
			{
			  base = trio_to_long(&format[offset],
					      &tmpformat,
					      BASE_DECIMAL);
			  if (base > MAX_BASE)
		  ...
		  if (TYPE_PRINT == type)
		    {
		      ...
		      width = TrioGetPosition(format, &offset);
		...
		case '5': case '6': case '7': case '8': case '9':
		   ...
		   */
		  width = trio_to_long(&format[offset - 1],
				       &tmpformat,
				       BASE_DECIMAL);
		  offset = (int)(tmpformat - format);
		  break;
			...
			if (without_namespace)
			  {
			    ...
			    parameters[pos].type = FORMAT_PARAMETER;
			    parameters[pos].offsetAfterSpecifier = offset;
 ...
 reprocess:
  ...
  if (trio_floorl(number * dblFractionBase) != trio_floorl(workNumber))
      ...
      if ((flags & FLAGS_FLOAT_G) && !(flags & FLAGS_FLOAT_E))
	{
      ...
      if (flags & FLAGS_FLOAT_E)
	{
	  workDigits = 1 + (int)TrioLogarithm(trio_floorl(workNumber), base);
	   ...
	   trio_parameter_t *parameters)
{
  ...
  while (format[offset])
      ...
      if (! isascii(format[offset]))
	{
      ...
      if (CHAR_IDENTIFIER == format[offset])
	{
	  if (CHAR_IDENTIFIER == format[offset + 1])
	    {
		...
		default:
	      ...
	      /* Prepare for next */
	      offset = parameters[i].offsetAfterSpecifier;
	      i++;
	    }
	}
      else /* not identifier */
