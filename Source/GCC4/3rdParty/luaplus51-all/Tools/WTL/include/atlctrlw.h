...
// Methods
	...
	BOOL AddIcon(_U_STRINGorID icon, UINT nCommandID)
	{
		ATLASSERT(::IsWindow(m_hWnd));
	...
	BOOL ReplaceIcon(_U_STRINGorID icon, UINT nCommandID)
	{
		ATLASSERT(::IsWindow(m_hWnd));
		HICON hIcon = ::LoadIcon(_Module.GetResourceInstance(), icon.m_lpstr);
		if(hIcon == NULL)
		...
		if(pnmh->hwndFrom == m_hWnd)
		{
			...
			else if(lpTBCustomDraw->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
			{
				if(m_bFlatMenus)
				{
					...
					tbbi.pszText = szText;
					tbbi.cchText = sizeof(szText) / sizeof(TCHAR);
...
// Note: If this is used in a rebar, all menus should have the same top level items.
// If you have different menus, you'll have to set the rebar band info when they switch.
...
// Constructor/destructor
	CMDICommandBarCtrlImpl() : m_wndMDIClient(this, 2), m_bChildMaximized(false), 
...
// Additional MDI message handlers
	...
	LRESULT OnNcPaint(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& /*bHandled*/)
	{
		...
		RECT rcIcon;
		T* pT = static_cast<T*>(this);
		pT->_CalcIconRect(cyHeight, rcIcon);
	...
	LRESULT OnNcLButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
	{
		...
		RECT rcIcon;
		T* pT = static_cast<T*>(this);
		pT->_CalcIconRect(rect.bottom - rect.top, rcIcon);
		RECT arrRect[3];
		...
		if(::PtInRect(&rcIcon, pt))
		{
			...
			UINT uRet = (UINT)menu.TrackPopupMenu(TPM_LEFTBUTTON | TPM_VERTICAL | TPM_LEFTALIGN | TPM_TOPALIGN | TPM_VERPOSANIMATION | TPM_RETURNCMD, 
				rect.left, rect.bottom, m_hWndChildMaximized);
	...
	LRESULT OnAllHookMessages(UINT uMsg, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
	{
		...
		if(m_bChildMaximized)
		{
			if(m_hWndChildMaximized != hWndChild)
			{
				...
				{
					TCHAR szClass[200];
					::GetClassName(wnd, szClass, sizeof(szClass) / sizeof(TCHAR));
					WNDCLASSEX wc;
					wc.cbSize = sizeof(WNDCLASSEX);
					// try process local class first
					BOOL bRet = ::GetClassInfoEx(_Module.GetModuleInstance(), szClass, &wc);
					if(!bRet)	// try global class
						bRet = ::GetClassInfoEx(NULL, szClass, &wc);
					m_hIconChildMaximized = bRet ? wc.hIcon : NULL;
				}
			}
		}
		else
...
// Implementation
	...
	void _CalcIconRect(int cyHeight, RECT& rect)
	{
	...
	void _CalcBtnRects(int cxWidth, int cyHeight, RECT arrRect[3]) const
	{
...
#ifndef _WTL_NO_AUTO_THEME
		if(m_hTheme != NULL)
			::OffsetRect(&rcBtn, -m_cxBtnWidth, 0);
		else
#endif //!_WTL_NO_AUTO_THEME
			::OffsetRect(&rcBtn, -(m_cxBtnWidth + m_cxyOffset), 0);
		arrRect[1] = rcBtn;
		::OffsetRect(&rcBtn, -m_cxBtnWidth, 0);
		arrRect[2] = rcBtn;
	}
