...
{
	if (m_hWnd) 
		RawHookWindow((HWND)NULL);		// unhook window
}
...
// pWnd=NULL to remove.
//
BOOL CSubclassWnd::RawHookWindow(HWND hwnd)
{
	ASSERT_VALID(this);
	if (hwnd) {
		// Hook the window
...
void CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pSubclassWnd)
{
	...
	if (pSubclassWnd->m_pNext==NULL) {
		...
		pSubclassWnd->m_pOldWndProc = 
			(WNDPROC)SetWindowLong(hwnd, GWLP_WNDPROC, (DWORD)HookWndProc);
...
void CSubclassWndMap::Remove(CSubclassWnd* pUnHook)
{
	...
	if (pHook==pUnHook) {
		...
		if (pHook->m_pNext)
			...
			RemoveKey(hwnd);
			SetWindowLong(hwnd, GWLP_WNDPROC, (DWORD)pHook->m_pOldWndProc);
		}
	} else {
...
void CSubclassWndMap::RemoveAll(HWND hwnd)
{
	...
	while ((pSubclassWnd = Lookup(hwnd))!=NULL)
		pSubclassWnd->RawHookWindow((HWND)NULL);	// (unhook)
}
